...
*   **⚙️ Механизм:** Как и в случае с другими командами, использующими динамические библиотеки, `scp` также можно подвергнуть атаке `LD_PRELOAD`. Злоумышленник создает вредоносную библиотеку (`.so`), которая перехватывает системные вызовы, используемые `scp` для чтения/записи файлов, или функции SSH-клиента, ответственные за аутентификацию и передачу данных. Затем эта библиотека загружается перед всеми остальными с помощью `LD_PRELOAD`, и `scp` начинает использовать вредоносные функции.*   **👨‍💻 Пример (Псевдокод для концепции):**
    ```c
