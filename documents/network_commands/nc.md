...
1.  **Сканування портів (простий аналог nmap):**
    ...
    *Вивід: `Connection to google.com 80 port [tcp/http] succeeded!`*
    ...
    ```bash
    # Перевірити, чи відкритий порт 21 (FTP) на IP-адресі 192.168.1.10
    nc -zv 192.168.1.10 21
    ```
    *Використання `-z` та `-v` дозволяє швидко перевірити доступність порту 21 (який часто використовується для FTP) на вказаній IP-адресі, не надсилаючи жодних даних. Це корисно для діагностики мережі та перевірки, чи працює служба на певному порті.*
...
8.  **Надсилання HTTP GET запиту:**
    ```bash
    echo -e "GET / HTTP/1.0\r\n\r\n" | nc 192.168.1.10 80
    ```
    *Надсилає HTTP GET запит до веб-сервера на IP-адресі 192.168.1.10, порт 80, і відображає відповідь сервера. Цей приклад демонструє, як `nc` може бути використаний для базового тестування HTTP-серверів або для ручного створення HTTP-запитів.*
